...
// process bookings by student or bookothers
    ...
    if($bookings) {
        foreach($bookings as $key => $booking) {
            ...
            if($records = $DB->get_records('examregistrar_bookings', array('examid'=>$booking['examid'], 'userid'=>$userid,
                ...
                if(!$exam) {
                    ...
                    if(($exam->period == $period) &&
                            ...
                            (examregistrar_check_exam_within_period($now, $periodobj, $examdate, $config) OR $canmanageexams)) {
            ...
            examregistrar_booking_clean_newbookings($examregistrar->id, $booking, $newid, $userid, $context);
        }
    }
