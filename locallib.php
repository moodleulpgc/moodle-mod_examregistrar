 ...
 * @return void of properly built examdelivery records
 */
function examregistrar_exam_addupdate_delivery_formdata($examid, $formdata, $eventdata) {
    global $CFG, $DB; 
    ...
    foreach(range(0, $formdata->deliver_repeats - 1) as $index) {
        if($formdata->helpercmid[$index] || $formdata->deliveryid[$index]) {
            ...
            foreach(range(0, $parcount-1) as $i) {
                ...
                print_object("i= $i"); 
                print_object("par  ".$formdata->{$parameter}[$index]);
                print_object("val  ".$formdata->{$value}[$index]);
            ...
            print_object("Index = $index");
            print_object($parameters);
            
            if($parameters) {
                $delivery->parameters = serialize($parameters);
            }
            $deliveryexams[$index] = $delivery;
        }
    }
    
    print_object($deliveryexams);
    die;
    ...
    foreach($deliveryexams as $delivery) {
        ...
        $helpermod = $DB->get_record($delivery->helpermod, array('id' => $instanceid), '*', MUST_EXIST);
...
function examregistrar_process_exam_delivery_user_override($delivery, $userid) {
    ...
    if(!isset($delivery->instanceid)) {
        $delivery->instanceid = $DB->get_field($delivery->helpermod);
    }
    ...
    $done = '';
    if($delivery->helpermod == 'quiz') {
        if(!$overrride = $DB->get_record('quiz_overrides', ['quiz' => $delivery->instanceid, 'userid' => $userid])) {
            $overrride = new stdClass();
            $overrride->quiz = $delivery->instanceid;
            $overrride->userid = $userid;
        }
        if(isset($delivery->timeopen) && $delivery->timeopen) {
            $overrride->timeopen = $delivery->timeopen;
        }
        if(isset($delivery->timeclose) && $delivery->timeclose) {
            $overrride->timeclose = $delivery->timeclose;
        }
        if(isset($delivery->timeclose) && $delivery->timeclose) {
            $overrride->timeclose = $delivery->timeclose;
        }
        ...
        if(isset($parameters['quizpassword'])) {
            $overrride->password = $delivery->$parameters['quizpassword'];
        }
        if(isset($parameters['attempts'])) {
            $overrride->attempts = $delivery->$parameters['attempts'];
        }
    } elseif($delivery->helpermod == 'assign') {
        if(!$overrride = $DB->get_record('assign_overrides', ['assignid' => $delivery->instanceid, 'userid' => $userid])) {
            $overrride = new stdClass();
            $overrride->assignid = $delivery->instanceid;
            $overrride->userid = $userid;
        }
        if(isset($delivery->timeopen) && $delivery->timeopen) {
            $overrride->allowsubmissionsfromdate = $delivery->timeopen;
        }
        if(isset($delivery->timeclose) && $delivery->timeclose) {
            $overrride->cutoffdate = $delivery->timeclose;
        }
        if(isset($delivery->timelimit) && $delivery->timelimit) {
            $overrride->duedate = $delivery->timeopen + $delivery->timelimit;
        }
    }
    
    if(isset($overrride->userid)) {
        $table = $deliver->helpermod.'_overrides';
        if(isset($overrride->id)) {
 ...
 * @return array ($select, params) tuple for get_records_xx database functions
 */
function examregistrar_decode_examquiz_idnumber($exregid, $courseid, $idnumber) {
    global $DB;
